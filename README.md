...
- `gakumasu-timeline`: 学マスのコミュや出来事を時間軸で確認できる年表アプリ（編集中）
...
`.github/workflows/deploy-pages.yml` は、このポータルに加えて `gakumasu-timeline` を checkout して GitHub Pages 用の成果物を作成します。

`supportcard-status` は Cloudflare Workers で公開済みの外部ツールとして扱い、ポータルから `https://supportcard-status-calculate.curiretas.workers.dev/` へリンクします。

`rectaris/calc-sapo` は GitHub Pages workflow では checkout しません。そのため `supportcard-status` 用の `CROSS_REPO_READ_TOKEN` は不要です。
